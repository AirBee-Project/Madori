...
export default function hyperVoxelParse(
	...
	const voxelStringList: string[] = voxelsString
		.split(",")
		.filter((v) => v.trim() !== "");
	const result: VoxelDefinition[] = [];
	...
	for (let i = 0; i < voxelStringList.length; i++) {
		...
		const voxelParseList = spatialPart.split("/");
		const zValue: number = Number(voxelParseList[0]);

		const { startTime, endTime } = parseTimePart(timePart);

		const resultVoxel: VoxelDefinition = {
			Z: zValue,
