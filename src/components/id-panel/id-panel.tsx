 ...
 */
function IdBox({ item, onUpdate, onDelete, onFocus, onColorClick }: IdBoxProps) {
	return (
...
export default function IdPanel({
	...
	const getActiveColor = (): [number, number, number, number] => {
	 ...
	 　　* カラーピッカーを開いているID一つ
	 　　*/
		const activeItem = items.find((i) => i.id === openPickerId) as Item<"voxel"> | undefined;
		/**
	 　　* カラーピッカーを開いているIDの色
	 　　*/
		const activeColor = activeItem?.data?.color as [number, number, number, number] | undefined;
