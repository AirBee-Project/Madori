...
const TimeAxis: React.FC<TimeAxisProps> = ({
    ...
    const [viewCenter, setViewCenter] = useState(currentTime > 0 ? currentTime : 1735689600);
    ...
    useEffect(() => {
        ...
        for (let t = firstTick; t <= endTime; t += tickInterval) {
            ...
            else if (tickInterval >= 2628000) label = `${date.getMonth() + 1}/${date.getDate()}`;
            else if (tickInterval >= 86400) label = `${date.getMonth() + 1}/${date.getDate()}`;
            else label = `${date.getHours()}:${date.getMinutes().toString().padStart(2, '0')}`;
